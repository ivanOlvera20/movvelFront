...
export default function Modelo() {
  ...
  const [fetch, refetch] = useGet('/modelo');
  const [put] = usePut('/modelo');
  const [del] = useDelete('/modelo');
  const [post] = usePost('/modelo');
  ...
  return (
    <>
      <PageTitle title="Modelo" button="Añadir modelo" onClick={() => setCollapse((prev) => !prev)} />
