...
export default function Familia() {
  ...
  const [newfam, setFamFields] = useState({
    descripcion: '',
  });
  ...
  const [fetch, refetch] = useGet('/familia');
  const [put] = usePut('/familia');
  const [del] = useDelete('/familia');
  const [post] = usePost('/familia');
  ...
  return (
    <>
      <PageTitle title="Familia" button="Añadir familia" onClick={() => setCollapse((prev) => !prev)} />
              ...
              <Grid container spacing={3}>
                <Grid item xs={8}>
                  <TextField
                    name="descripcion"
                    fullWidth
                    label="Descripcion de la familia"
                    variant="outlined"
                    value={newfam.nombre}
                    onChange={(e) => setFamFields((prev) => ({
                      ...prev,
