...
export default function Banco() {
  ...
  const [fetch, refetch] = useGet('/banco');
  const [put] = usePut('/banco');
  const [del] = useDelete('/banco');
  const [post] = usePost('/banco');
  ...
  return (
    <>
      <PageTitle title="Bancos" button="Añadir Banco" onClick={() => setCollapse((prev) => !prev)} />
